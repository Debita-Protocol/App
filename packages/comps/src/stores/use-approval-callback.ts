...
export function useApproveCallback(
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    return isApproved ? (pendingApproval ? ApprovalState.PENDING : ApprovalState.NOT_APPROVED) : ApprovalState.APPROVED;
...
export const hasPendingTransaction = async (
  ...
  const tx = Object.values(transactions).find(
    (tx) =>
      tx.approval && !tx.receipt && tx?.approval?.spender === spender && tx?.approval?.tokenAddress === tokenAddress
  );
  return Boolean(tx);
};
...
export async function checkAllowance(
  ...
  const isPending = await hasPendingTransaction(transactions, account, tokenAddress, spender);
...
export const checkIsERC20Approved = async (
  ...
  const allowance = await getERC20Allowance(erc20TokenAddress, library, account, spender);
...
export const isERC1155ContractApproved = async (
  ...
  return isApproved ? ApprovalState.APPROVED : isPending ? ApprovalState.PENDING : ApprovalState.NOT_APPROVED;
};
