...
library config{
  ...
  uint256 public constant roundLimit = 3; 
  ...
  function convertToWad(uint256 number, uint256 dec) public pure returns(uint256 new_number){
    //number should not be 18 dec, but in collateral_dec
    new_number = number * (10 ** (WAD_PRECISION - dec));
    assert(new_number <= max_amount); 
  }

  function wadToDec(uint256 number, uint256 dec) public pure returns(uint256 new_number){
  ...
  function isInWad(uint256 number) public pure returns(bool){
